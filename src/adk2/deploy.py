...
from . import agent_engine_executor
...
def main():
    ...
    agent_skill = AgentSkill(
        ...
        description="An internal assistant for insurance company employees that can search policies.",
        tags=["Insurance", "Policies"],
        examples=[
            "Search for car insurance policies.",
            "Tell me about the 'Comprehensive Gold' policy.",
            "List all available insurance tariffs.",
        ],
    )

    insurance_agent_card = create_agent_card(
        agent_name="Insurance Assistant Agent",
        description="A helpful assistant for insurance policy questions.",
        skills=[agent_skill],
        streaming=False,
    ...
    print(f"✓ Insurance assistant agent card created. {insurance_agent_card}")

    a2ui_agent = A2aAgent(
        agent_card=insurance_agent_card,
        agent_executor_builder=agent_engine_executor.AgentEngineExecutor,
        agent_executor_kwargs={"agent_card": insurance_agent_card},
    )
    a2ui_agent.set_up()

    print("✓ Local Insurance assistant agent created.")

    config = {
        "display_name": "Insurance Assistant Agent on Agent Engine",
        "description": (
            "An AI assistant for insurance policy questions and claims."
        ),
    ...
    enterprise_agent = _register_agent_on_gemini_enterprise(
        ...
        agent_name="insurance_assistant_agent",
        display_name="Insurance Assistant Agent",
        description="An AI assistant for insurance policy questions and claims.",
...
if __name__ == "__main__":
    # Load .env from the parent directory if run from src/adk2
